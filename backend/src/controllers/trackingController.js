...
// Captura sessão de tracking (chamado pela landing page)
async function capture(req, res) {
  ...
  // Resolve campaign_id se tiver gclid ou utm_campaign
  let campaignId = null;
  ...
  // Salva no Redis para lookup rápido por GCLID
  if (utmParams.gclid) {
    await redis.set(`gclid:${utmParams.gclid}`, rows[0].id, 86400 * 90);
  }
  if (utmParams.fbclid) {
    await redis.set(`fbclid:${utmParams.fbclid}`, rows[0].id, 86400 * 90);
  }
  await redis.set(`session:${sessionId}`, rows[0].id, 86400 * 30);

  res.json({ session_id: sessionId, tracking_id: rows[0].id });
}

// Evento de analytics (page view, click, etc.)
async function event(req, res) {
  ...
  `, [
    uuidv4(), session_id, trackingId?.id || null,
...
// Resolve tracking por GCLID (para conversões offline)
...
async function trafficDashboard(req, res) {
  ...
  const [sessionsRes, sourceRes, deviceRes, keywordRes] = await Promise.all([
    ...
    `, [df, dt]),
    db.query(`
      SELECT utm_term as keyword, COUNT(*) as sessions, COUNT(DISTINCT l.id) as leads
      FROM utm_tracking ut
      LEFT JOIN leads l ON l.utm_tracking_id = ut.id
      WHERE ut.utm_term IS NOT NULL AND ut.first_seen_at BETWEEN $1 AND $2
      GROUP BY utm_term ORDER BY leads DESC LIMIT 20
    `, [df, dt]),
  ]);
